...
function Test-ClientSecretFormat {
    ...
    # Azure client secrets typically start with specific patterns
    if ($Value -match "^[a-zA-Z0-9~._-]{30,}$") {
...
function Test-TenantConfig {
    ...
    # Validate structure
    Write-Status "`n📋 Validating JSON Structure..." "Info"
    ...
    Write-Status "`n🏢 Validating Tenant Configurations..." "Info"
    
    if ($config.tenants) {
        foreach ($tenantKey in $config.tenants.PSObject.Properties.Name) {
            $tenant = $config.tenants.$tenantKey
            Write-Status "`nValidating tenant: $tenantKey" "Info"
    ...
    Write-Status "`n☁️  Validating Azure Communication Services..." "Info"
    ...
    # Validate Key Vault
    Write-Status "`n🔐 Validating Key Vault Configuration..." "Info"
    ...
    Write-Status "`n🔍 Performing Cross-Validation..." "Info"
...
try {
    ...
    if ($result) {
        Write-Status "`n🎉 Configuration validation completed successfully!" "Success"
        exit 0
    }
    else {
        Write-Status "`n💥 Configuration validation failed!" "Error"
        exit 1
    }
}
catch {
